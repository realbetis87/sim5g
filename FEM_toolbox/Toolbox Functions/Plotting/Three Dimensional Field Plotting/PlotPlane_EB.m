...
%}
%--------------------------------------------------------------------------
function [Ex,Ey,Ez,Bx,By,Bz,xx,yy,zz] = PlotPlane_EB(TModel,X,Position,Points),ElectromagneticConstants;
...
function [Ex,Ey,Ez,Bx,By,Bz,xx,yy,zz]=PlotXPlane_Field(TModel,EdgeVector,FacetVector,xx,yy,zz),error=1000*eps;empty=zeros(size(yy,1),size(yy,2));Ex=empty;Ey=empty;Ez=empty;Bx=empty;By=empty;Bz=empty;
    ...
    rEx=real(Ex);rEy=real(Ey);rEz=real(Ez);%[ii,jj]=find(abs(rEx)<1000*eps);rEx(ii,jj)=0;;[ii,jj]=find(abs(rEy)<1000*eps);rEy(ii,jj)=0;rEz=real(Ez);[ii,jj]=find(abs(rEz)<1000*eps);rEz(ii,jj)=0;
    iEx=imag(Ex);iEy=imag(Ey);iEz=imag(Ez);%[ii,jj]=find(abs(iEx)<1000*eps);iEx(ii,jj)=0;iEy=imag(Ey);[ii,jj]=find(abs(iEy)<1000*eps);iEy(ii,jj)=0;iEz=imag(Ez);[ii,jj]=find(abs(iEz)<1000*eps);iEz(ii,jj)=0;
    rBx=real(Bx);rBy=real(By);rBz=real(Bz);%[ii,jj]=find(abs(rBx)<1000*eps);rBx(ii,jj)=0;rBy=real(By);[ii,jj]=find(abs(rBy)<1000*eps);rBy(ii,jj)=0;rBz=real(Bz);[ii,jj]=find(abs(rBz)<1000*eps);rBz(ii,jj)=0;
    iBx=imag(Bx);iBy=imag(By);iBz=imag(Bz);%[ii,jj]=find(abs(iBx)<1000*eps);iBx(ii,jj)=0;iBy=imag(By);[ii,jj]=find(abs(iBy)<1000*eps);iBy(ii,jj)=0;iBz=imag(Bz);[ii,jj]=find(abs(iBz)<1000*eps);iBz(ii,jj)=0;
