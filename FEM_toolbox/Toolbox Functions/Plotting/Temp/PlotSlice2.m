function [Ex,Ey,Ez,Bx,By,Bz,xx,yy,zz] = PlotSlice2(XU,XK,Position,Points),global Edges;global Facets;global NumberOfFacets;global NumberOfEdges;global ComputationalDomain;Constants;
%--------------------------------------------------------------------------
%{
...
function [Ex,Ey,Ez,Bx,By,Bz,xx,yy,zz]=PlotXPlane(EdgeVector,FacetVector,xx,yy,zz,N1,N2),global Elements;global Edges;global Facets;global error;empty=zeros(N1,N2);Ex=empty;Ey=empty;Ez=empty;Bx=empty;By=empty;Bz=empty;
    ...
    %}
    figure;
    subplot(2,3,1);pcolor(abs(Ex));shading interp;hold on;colormap(cmap);title('x Component Electric Field Abs');colorbar;axis tight;axis equal;
    subplot(2,3,2);pcolor(abs(Ey));shading interp;hold on;colormap(cmap);title('y Component Electric Field Abs');colorbar;axis tight;axis equal;
    subplot(2,3,3);pcolor(abs(Ez));shading interp;colormap(cmap);title('z Component Electric Field Abs');colorbar;axis tight;axis equal;
    subplot(2,3,4);pcolor(abs(Bx));shading interp;hold on;colormap(cmap);title('x Component Magnetic Flux Abs');colorbar;axis tight;axis equal;
    subplot(2,3,5);pcolor(abs(By));shading interp;hold on;colormap(cmap);title('y Component Magnetic Flux Abs');colorbar;axis tight;axis equal;
    subplot(2,3,6);pcolor(abs(Bz));shading interp;colormap(cmap);title('z Component Magnetic Flux Abs');colorbar;axis tight;axis equal;
end
