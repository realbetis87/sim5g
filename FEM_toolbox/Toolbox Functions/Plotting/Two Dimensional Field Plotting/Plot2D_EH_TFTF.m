function [] = Plot2D_EH_TFTF(TModel,BoundaryExcitation,Nhor,Nver,X)
    NEdges=numel(BoundaryExcitation.Edges);EdgeVector_E=zeros(NEdges,1);EdgeVector_H=zeros(NEdges,1);
    ...
    Ex=zeros(Nver,Nhor);Ey=zeros(Nver,Nver);Ez=zeros(Nver,Nhor);
    Hx=zeros(Nver,Nhor);Hy=zeros(Nver,Nver);Hz=zeros(Nver,Nhor);
    ...
    switch abs(boundary.Axis)
        case 1,y=linspace(Ymin-Ymin*1e-3,Ymax-Ymax*1e-3,Nhor);z=linspace(Zmin-Zmin*1e-3,Zmax-Zmax*1e-3,Nver);[yy,zz]=meshgrid(y,z);Plot2D_EH_TFTF_x(TModel,BoundaryExcitation,Ey,Ez,Hy,Hz,EdgeVector_E,EdgeVector_H,yy,zz);
        case 2,x=linspace(Xmin-Xmin*1e-3,Xmax-Xmax*1e-3,Nhor);z=linspace(Zmin-Zmin*1e-3,Zmax-Zmax*1e-3,Nver);[xx,zz]=meshgrid(x,z);Plot2D_EH_TFTF_y(TModel,BoundaryExcitation,Ex,Ez,Hx,Hz,EdgeVector_E,EdgeVector_H,xx,zz);
        case 3,x=linspace(Xmin-Xmin*1e-3,Xmax-Xmax*1e-3,Nhor);y=linspace(Ymin-Ymin*1e-3,Ymax-Ymax*1e-3,Nver);[xx,yy]=meshgrid(x,y);Plot2D_EH_TFTF_z(TModel,BoundaryExcitation,Ex,Ey,Hx,Hy,EdgeVector_E,EdgeVector_H,xx,yy);
    end
end
%--------------------------------------------------------------------------
function [] = Plot2D_EH_TFTF_x(TModel,BoundaryExcitation,Ey,Ez,Hy,Hz,EdgeVector_E,EdgeVector_H,yy,zz),ci=0;
    ...
    subplot(2,2,1);pcolor(yy,zz,real(Ey));shading interp;colormap(cmap);title('y Component Electric Field');colorbar;axis tight;axis equal;
    subplot(2,2,2);pcolor(yy,zz,real(Ez));shading interp;colormap(cmap);title('z Component Electric Field');colorbar;axis tight;axis equal;
    subplot(2,2,3);pcolor(yy,zz,real(Hy));shading interp;colormap(cmap);title('y Component Magnetic Field');colorbar;axis tight;axis equal;
    subplot(2,2,4);pcolor(yy,zz,real(Hz));shading interp;colormap(cmap);title('z Component Magnetic Field');colorbar;axis tight;axis equal;
    [cmap]=buildcmap('cbkry');figure;title("Imaginary Fields");
    subplot(2,2,1);pcolor(yy,zz,imag(Ey));shading interp;colormap(cmap);title('y Component Electric Field');colorbar;axis tight;axis equal;
    subplot(2,2,2);pcolor(yy,zz,imag(Ez));shading interp;colormap(cmap);title('z Component Electric Field');colorbar;axis tight;axis equal;
    subplot(2,2,3);pcolor(yy,zz,imag(Hy));shading interp;colormap(cmap);title('y Component Magnetic Field');colorbar;axis tight;axis equal;
    subplot(2,2,4);pcolor(yy,zz,imag(Hz));shading interp;colormap(cmap);title('z Component Magnetic Field');colorbar;axis tight;axis equal;
    [cmap]=buildcmap('cbkry');figure;title("Absolute Fields");
    subplot(2,2,1);pcolor(yy,zz,abs(Ey));shading interp;colormap(cmap);title('y Component Electric Field');colorbar;axis tight;axis equal;
    subplot(2,2,2);pcolor(yy,zz,abs(Ez));shading interp;colormap(cmap);title('z Component Electric Field');colorbar;axis tight;axis equal;
    subplot(2,2,3);pcolor(yy,zz,abs(Hy));shading interp;colormap(cmap);title('y Component Magnetic Field');colorbar;axis tight;axis equal;
    subplot(2,2,4);pcolor(yy,zz,abs(Hz));shading interp;colormap(cmap);title('z Component Magnetic Field');colorbar;axis tight;axis equal;
end
function [] = Plot2D_EH_TFTF_y(TModel,BoundaryExcitation,Ex,Ez,Hx,Hz,EdgeVector_E,EdgeVector_H,xx,zz),ci=0;
    ...
    [cmap]=buildcmap('cbkry');figure;title("Real Fields");
    subplot(2,2,1);pcolor(yy,zz,real(Ex));shading interp;colormap(cmap);title('x Component Electric Field');colorbar;axis tight;axis equal;
    subplot(2,2,2);pcolor(yy,zz,real(Ez));shading interp;colormap(cmap);title('z Component Electric Field');colorbar;axis tight;axis equal;
    subplot(2,2,3);pcolor(yy,zz,real(Hx));shading interp;colormap(cmap);title('x Component Magnetic Field');colorbar;axis tight;axis equal;
    subplot(2,2,4);pcolor(yy,zz,real(Hz));shading interp;colormap(cmap);title('z Component Magnetic Field');colorbar;axis tight;axis equal;
    [cmap]=buildcmap('cbkry');figure;title("Imaginary Fields");
    subplot(2,2,1);pcolor(yy,zz,imag(Ex));shading interp;colormap(cmap);title('x Component Electric Field');colorbar;axis tight;axis equal;
    subplot(2,2,2);pcolor(yy,zz,imag(Ez));shading interp;colormap(cmap);title('z Component Electric Field');colorbar;axis tight;axis equal;
    subplot(2,2,3);pcolor(yy,zz,imag(Hx));shading interp;colormap(cmap);title('x Component Magnetic Field');colorbar;axis tight;axis equal;
    subplot(2,2,4);pcolor(yy,zz,imag(Hz));shading interp;colormap(cmap);title('z Component Magnetic Field');colorbar;axis tight;axis equal;
    [cmap]=buildcmap('cbkry');figure;title("Absolute Fields");
    subplot(2,2,1);pcolor(yy,zz,abs(Ex));shading interp;colormap(cmap);title('x Component Electric Field');colorbar;axis tight;axis equal;
    subplot(2,2,2);pcolor(yy,zz,abs(Ez));shading interp;colormap(cmap);title('z Component Electric Field');colorbar;axis tight;axis equal;
    subplot(2,2,3);pcolor(yy,zz,abs(Hx));shading interp;colormap(cmap);title('x Component Magnetic Field');colorbar;axis tight;axis equal;
    subplot(2,2,4);pcolor(yy,zz,abs(Hz));shading interp;colormap(cmap);title('z Component Magnetic Field');colorbar;axis tight;axis equal;
end
function [] = Plot2D_EH_TFTF_z(TModel,BoundaryExcitation,Ex,Ey,Hx,Hy,EdgeVector_E,EdgeVector_H,xx,yy),ci=0;
    ...
    [cmap]=buildcmap('cbkry');figure;title("Real Fields");
    subplot(2,2,1);pcolor(yy,zz,real(Ex));shading interp;colormap(cmap);title('x Component Electric Field');colorbar;axis tight;axis equal;
    subplot(2,2,2);pcolor(yy,zz,real(Ey));shading interp;colormap(cmap);title('y Component Electric Field');colorbar;axis tight;axis equal;
    subplot(2,2,3);pcolor(yy,zz,real(Hx));shading interp;colormap(cmap);title('x Component Magnetic Field');colorbar;axis tight;axis equal;
    subplot(2,2,4);pcolor(yy,zz,real(Hy));shading interp;colormap(cmap);title('y Component Magnetic Field');colorbar;axis tight;axis equal;
    [cmap]=buildcmap('cbkry');figure;title("Imaginary Fields");
    subplot(2,2,1);pcolor(yy,zz,imag(Ex));shading interp;colormap(cmap);title('x Component Electric Field');colorbar;axis tight;axis equal;
    subplot(2,2,2);pcolor(yy,zz,imag(Ey));shading interp;colormap(cmap);title('y Component Electric Field');colorbar;axis tight;axis equal;
    subplot(2,2,3);pcolor(yy,zz,imag(Hx));shading interp;colormap(cmap);title('x Component Magnetic Field');colorbar;axis tight;axis equal;
    subplot(2,2,4);pcolor(yy,zz,imag(Hy));shading interp;colormap(cmap);title('y Component Magnetic Field');colorbar;axis tight;axis equal;
    [cmap]=buildcmap('cbkry');figure;title("Absolute Fields");
    subplot(2,2,1);pcolor(yy,zz,abs(Ex));shading interp;colormap(cmap);title('x Component Electric Field');colorbar;axis tight;axis equal;
    subplot(2,2,2);pcolor(yy,zz,abs(Ey));shading interp;colormap(cmap);title('y Component Electric Field');colorbar;axis tight;axis equal;
    subplot(2,2,3);pcolor(yy,zz,abs(Hx));shading interp;colormap(cmap);title('x Component Magnetic Field');colorbar;axis tight;axis equal;
    subplot(2,2,4);pcolor(yy,zz,abs(Hy));shading interp;colormap(cmap);title('y Component Magnetic Field');colorbar;axis tight;axis equal;
end
