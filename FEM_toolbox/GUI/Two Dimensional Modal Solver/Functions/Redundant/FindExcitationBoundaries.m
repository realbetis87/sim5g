function [] = FindExcitationBoundaries(app),boundary=app.mainApp.TModel.Boundaries(app.mainApp.CurrentBoundaryIndex);error=1000*eps;
    if(boundary.Type=="POR")
            d1=[app.mainApp.TModel.Boundaries.Type]=="POR";d2=[app.mainApp.TModel.Boundaries.Axis]==boundary.Axis;d3=zeros(numel(app.mainApp.TModel.Boundaries),1);
            for ii=1:numel(app.mainApp.TModel.Boundaries),if(~isempty(app.mainApp.TModel.Boundaries(ii).Position)),if(abs(app.mainApp.TModel.Boundaries(ii).Position-boundary.Position)<=error),d3(ii)=1;end,end
            end,d=d1 & d2 & d3';boundaryIndices=find(d);app.BoundaryIndices=boundaryIndices;
    elseif(boundary.Type=="DIR")
            d1=[app.mainApp.TModel.Boundaries.Type]=="DIR";d2=[app.mainApp.TModel.Boundaries.Axis]==boundary.Axis;d3=zeros(numel(app.mainApp.TModel.Boundaries),1);
            for ii=1:numel(app.mainApp.TModel.Boundaries),if(~isempty(app.mainApp.TModel.Boundaries(ii).Position)),if(abs(app.mainApp.TModel.Boundaries(ii).Position-boundary.Position)<=error),d3(ii)=1;end,end
            end,d=d1 & d2 & d3;boundaryIndices=find(d);app.BoundaryIndices=boundaryIndices;
    end,app.mainApp.TModel=Prepare2DBoundaries(app.mainApp.TModel,boundaryIndices);app.TModel=app.mainApp.TModel;
end
